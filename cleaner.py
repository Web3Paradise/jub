...
def calculate_chunk_size(pattern_length):
    ...
    if pattern_length <= 3:
        ...
        return 500000  # Increased for 4-5 character patterns
    else:
        return 250000  # Increased for longer patterns too
...
def worker_process(args):
    ...
    worker_id, prefix, suffix, chunk_size, batch_size, progress_update = args
    ...
    while attempts < chunk_size:
        ...
        # Periodically report progress if requested
        if progress_update and time.time() - last_progress_time > progress_update:
            elapsed = time.time() - start_time
            rate = attempts / elapsed if elapsed > 0 else 0
...
def main():
    ...
    parser.add_argument("--progress", action="store_true", help="Show detailed worker progress")
    ...
    # Progress update interval for workers (0 = no progress updates)
    progress_update = args.status_interval if args.progress else 0
    ...
    try:
        ...
        while args.max_rounds == 0 or search_round < args.max_rounds:
            search_round += 1
            print(f"[INFO] Starting search round {search_round}")
            ...
            with ProcessPoolExecutor(max_workers=args.threads) as executor:
                ...
                for i in range(args.threads):
                    worker_args = (i, args.prefix, args.suffix, args.chunk_size, args.batch_size, progress_update)
                ...
                for future in as_completed(future_to_worker):
                    ...
                    try:
                        ...
                        if current_time - last_status_time >= args.status_interval:
                            ...
                            print(f"\r[INFO] Total attempts: {total_attempts:,}, Rate: {rate:.2f} keys/sec{progress_str}", end="")
            ...
            elapsed = time.time() - start_time
            if worker_stats:
                total_rate = sum(w["rate"] for w in worker_stats)
                avg_rate = total_rate / len(worker_stats)
                print(f"\r[INFO] Average processing rate: {avg_rate:.2f} keys/sec ({total_rate:.2f} total)", flush=True)
